...
/* temp buffer for background image */
uint32_t bg_image[HEIGHT][WIDTH];
...
void read_bg_image() {
  int  i, j, calc_w, calc_h, x, y;
	calc_w = TM_LCD_GetWidth() / 2;
	calc_h = TM_LCD_GetHeight() / 2;
	x = calc_w-(WIDTH/2);
	y = calc_h-(HEIGHT/2);
	for ( i = 0; i < HEIGHT; i++ ) {    
		for ( j = 0; j < WIDTH; j++ ) {							  
			  bg_image[i][j] = TM_LCD_GetPixel(x+j,y+i);			
		}
  }
}
...
void draw_image(uint16_t x, uint16_t y, uint32_t color) {
  ...
  {    
		for ( j = 0; j < WIDTH; j++ ) {				
			  ...
			  bg_rgb565 = bg_image[i][j]; /* TM_LCD_GetPixel(x+j,y+i); */
...
void print_freetype(char str[], int size) {
	int calc_h = 0, calc_w = 0;
  /* Try to mount card */
	if (f_mount(&FS, "SD:", 1) == FR_OK) {
		...
		if (init_freetype() == 0) {
			...
			if (draw_freetype(str, size, calc_w-(WIDTH/2), calc_h-(HEIGHT/2), 0xffff) == 1)
