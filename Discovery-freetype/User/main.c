...
//	TM_LCD_Fill(0xffff);
	...
	if (f_mount(&FS, "SD:", 1) == FR_OK) {
		...
		if (init_freetype() == 0) {
			...
			if (draw_freetype("This is a test to", 17, 0, 0, 0xffff) == 1)
				printf("\r\nError in reading/drawing routine.\r\n");
			if (draw_freetype("see if freetype", 15, 0, 80, 0xffff) == 1)
				printf("\r\nError in reading/drawing routine.\r\n");
			if (draw_freetype("correctly works.", 16, 0, 80*2, 0xffff) == 1)
				printf("\r\nError in reading/drawing routine.\r\n");
